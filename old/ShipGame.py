...
def score(ship):
    if ship.scuttle == True:
        return 0
    return (ship.money + (ship.goods * 5) )
...
def nextG(shipListInput):
    ...
    shipList = []
    for x in range(numberOfShips):
        shipNumberFromTopScorers = int(x % (int(len(shipScoreList) / 3) + 1))
        print(shipNumberFromTopScorers)
        chosenShip = shipScoreList[shipNumberFromTopScorers]
