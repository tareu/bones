	...
	def randomiseBiases(self):
		...
		for x in range(1, len(self.layers)): 
			...
			for y in range(len(self.layers[x])):
				...
				#here centre is as above. when making nect generation of nn, new bias should centre on best case random bias and half the area in which random numbers can come from. for instance if best case random bias is +12 the generated random numbers should come from the range random.random() * 20 (halving the range) + 2 (centering range on 12 +-10)
